...
void LoadUser(AstraDB *db) {
    countUser = 0;
    if (astra_query(db, "SELECT user_id, username, role FROM users")) {
...
void DrawTablePeminjaman(AstraDB *db, float x, float y, float w) {
    DrawHeader(x, y, w, "PEMINJAM", "BARANG", "TGL KEMBALI", "STATUS");
    for (int i = 0; i < countPinjam; i++) {
        float ry = y + 40 + (i * 45);
        DrawText(listPinjam[i].peminjam, (int)x+10, (int)ry, 20, BLACK);
        DrawText(listPinjam[i].barang, (int)x+100, (int)ry, 20, BLACK);
        
        bool telat = (listPinjam[i].hari_terlambat > 0);
        DrawText(listPinjam[i].tgl_rencana, (int)x+300, (int)ry, 20, telat?RED:BLACK);
        if(telat) DrawText(TextFormat("(+%d Hari)", listPinjam[i].hari_terlambat), (int)x+300, (int)ry+20, 12, RED);
        DrawText(listPinjam[i].status, (int)x+550, (int)ry, 20, BLUE);
...
AppState DrawDashboardAdmin(AstraDB *db, Texture2D logo) {
    ...
    if (showDetailPopup && detailIndex >= 0) {
        ...
        else if (currentMenu == MENU_USERS) { DRAW_ROW("ID User:", listUser[detailIndex].id); DRAW_ROW("Username:", listUser[detailIndex].username); DRAW_ROW("Role:", listUser[detailIndex].role); }
