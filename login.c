...
static bool DrawInputBox(float x, float y, const char* label, char* text, bool isActive, bool isPassword) {
    ...
    if (isActive) DrawRectangleRoundedLines(rect, 0.2f, 4, COL_ACCENT); 
    else DrawRectangleRoundedLines(rect, 0.2f, 4, LIGHTGRAY);
...
AppState CheckDatabaseLogin(const char *inputUser, const char *inputPass)
{
    ...
    if (astra_connect(&db, "borrowbee;UID=lat;PWD=lat123")) {
        char query[256];
        ...
        if (astra_query(&db, query)) {
            if (SQLFetch(db.hStmt) == SQL_SUCCESS) {
                ...
                if (strcmp(roleStr, "admin") == 0) nextState = STATE_DASHBOARD_ADMIN;
                else nextState = STATE_DASHBOARD_USER;
            }
        }
        astra_disconnect(&db);
    } else {
...
AppState DrawLoginScreen(Texture2D imgLogo)
{
    ...
    if (IsKeyPressed(KEY_TAB)) activeBox = !activeBox; 
    int key = GetCharPressed();
    char *target = (activeBox == 0) ? username : password;
    if (key > 0 || IsKeyPressed(KEY_BACKSPACE)) loginError = false;
    while (key > 0) {
        if ((key >= 32) && (key <= 125) && (strlen(target) < 63)) {
            int len = strlen(target); target[len] = (char)key; target[len + 1] = '\0';
        }
        key = GetCharPressed();
    }
    if (IsKeyDown(KEY_BACKSPACE) && IsKeyPressed(KEY_BACKSPACE)) Backspace(target);
    ...
    if (DrawInputBox(contentX, inputY, "Username", username, (activeBox == 0), false)) activeBox = 0;
    if (DrawInputBox(contentX, inputY + 90, "Password", password, (activeBox == 1), true)) activeBox = 1;
    ...
    if (hoverLink) {
        ...
        if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
            ...
            username[0] = '\0'; password[0] = '\0'; loginError = false;
    ...
    if (clickBtn) {
        ...
        if (result != STATE_LOGIN) {
            username[0] = '\0'; password[0] = '\0'; loginError = false; return result; 
        } else {
            loginError = true;
        }
    }
    ...
    if (IsKeyPressed(KEY_GRAVE)) {
        username[0]='\0'; password[0]='\0'; return STATE_MENU;
    }
